...
- While not exactly the reason for the war in Ukraine, agriculture became a target of the attackers.
- The war unfolded far from Africa or Asia, but pundits have warned against the real possibility that conflict and violence could likely follow in countries that rely on agricultural imports from this region.
]
...
.right-column[
```{r food_riots_image, fig.align="left", out.width = "70%", dpi=200}
...
.pull-right[
```{r conflict_map_image, out.width='80%'}
...
.right-column[
```{r series_prices_image, out.width='100%'}
...
.pull-right[
```{r map_harvest_image, out.width='80%'}`r`ninclude_graphics('figures/map_harvest.png')
```
]
...
.pull-right[
```{r circular_violence_image, out.width='80%'}`r`ninclude_graphics('figures/circular_violence_polit.png')
```
]
...
.pull-right[
```{r somalia_map_image, fig.align="left", out.width = "70%", dpi=200}`r`ninclude_graphics('figures/Somalia_Map.png') 
```
...
.right-column[
```{r price_transmission_image, out.width='100%'}`r`ninclude_graphics('figures/price_transmission.png')
```
]
