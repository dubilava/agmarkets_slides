...
subtitle: "Lecture 6: Storage and Futures Markets"
...
- A futures contract is a legal instrument, enforceable by the rules of the exchange on which it is traded, to deliver or accept delivery of a specific amount of a commodity during a specified month at a price established in the market.
...
- Futures contracts are a highly standardized form of forward contracts: 
  ...
  * wherein the agreement is backed by a good-faith deposit, referred to as 'margin.'
...
- The difference between spot and futures prices is referred to as the price of storage, which is the expected return from holding inventory. 
...
```{r storage, fig.cap='Supply-of-Storage', out.width='90%', fig.asp=0.6, dpi=200, fig.align='center', echo=FALSE, warning=FALSE, cache=TRUE}
...
ggplot(dt)+
  ...
  geom_line(aes(x=x,y=y3),size=1,color="indianred",linetype=5,na.rm=T)+
  ...
  theme_classic()+
  theme(axis.title = element_text(size=16), axis.text.x = element_blank(), axis.text.y = element_text(size=10),axis.ticks = element_blank(),plot.margin = margin(1,0,0,0,"cm"), axis.line.x= element_blank())
```
