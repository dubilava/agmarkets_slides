...
if (requireNamespace("cowplot", quietly = TRUE)) {
  ...
  plot_grid <- function(..., nrow = 1, ncol = NULL, labels = NULL, label_colour = NULL, hjust = NULL, align = NULL) {
    plots <- list(...)
    if (requireNamespace("gridExtra", quietly = TRUE)) {
      g <- do.call(gridExtra::arrangeGrob, c(plots, list(nrow = nrow, ncol = ncol)))
      grid::grid.newpage()
      grid::grid.draw(g)
      return(invisible(g))
    }
    warning("cowplot not installed; returning first plot only.")
    plots[[1]]
  }
}
...
.right-column[
```{r demand, out.width='100%', fig.asp=0.5, dpi=300, echo=FALSE, warning=FALSE, cache=FALSE}
...
.pull-left[
```{r marshall_photo, out.width = "300px"}
...
.pull-left[
```{r lancaster_photo, out.width = "300px"}
...
- Market demand is a schedule of quantities that all consumers in a market are willing to buy at given prices, *ceteris paribus*. 
